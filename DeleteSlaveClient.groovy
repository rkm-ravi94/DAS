job('Delete-Node') {
  ...
  steps {
		...
		'java -jar ${JENKINS_CLI_JAR} -s ${JENKINS_URL} delete-job ${LABEL}-Tomcat-Deployment --username "${USERNAME}" --password "${PASSWORD}"\n' +
                'java -jar ${JENKINS_CLI_JAR} -s ${JENKINS_URL} delete-job ${LABEL}-Setup-Logstash-Job --username "${USERNAME}" --password "${PASSWORD}"\n' +
                'java -jar ${JENKINS_CLI_JAR} -s ${JENKINS_URL} delete-job ${LABEL}-Setup-Setup-Postgresql-Job --username "${USERNAME}" --password "${PASSWORD}"\n' +
